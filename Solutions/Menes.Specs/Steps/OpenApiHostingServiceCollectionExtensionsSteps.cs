...
namespace Menes.Specs.Steps
{
    ...
    public class OpenApiHostingServiceCollectionExtensionsSteps
    {
        ...
        {
            ServiceDescriptor registration = this.scenarioContext.Get<ServiceCollection>().FirstOrDefault(x => x.ServiceType == typeof(IOpenApiHost<HttpRequest, IActionResult>));
        ...
        {
            IOpenApiExceptionMapper exceptionMapper = this.scenarioContext.Get<ServiceProvider>().GetRequiredService<IOpenApiExceptionMapper>();

            // The only way to tell if it's been mapped without reflecting into the guts of the mapper is to try and register a new mapper for
            // the given type/status code... even with this we need a little bit of reflection to invoke the method.
            var type = Type.GetType(exceptionType);
            MethodInfo mapMethod = exceptionMapper.GetType().GetMethods().First(x => x.Name == "Map" && x.GetGenericArguments().Length == 1);
            ...
            {
                // The exception will have been wrapped in a TargetInvocationException because we invoked via reflection, so we need to
                // see if the inner exception was what we expected.
        ...
        {
            bool expectedServiceIsRegistered = this.scenarioContext.Get<ServiceCollection>().Any(x => x.ServiceType == typeof(TService) && x.ImplementationType == typeof(TImplementation));
