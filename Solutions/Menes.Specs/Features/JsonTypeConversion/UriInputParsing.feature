...
Feature: Uri Input Parsing
    In order to implement a web API
    As a developer
    I want to be able to specify URI-formatted string parameters within the OpenAPI specification and have corresponding input parameters deserialized and validated

Scenario Outline: Body with valid values for simple types
    Given I have constructed the OpenAPI specification with a request body of type 'string', and format 'uri'
    When I try to parse the value '<Value>' as the request body
    Then the parameter body should be <ExpectedResult> of type System.Uri

    Examples:
        # Note: relative URIs have always been accepted as inputs (although not as outputs).
        # It's not clear that this is a correct interpretation of the specification, but it
        # is a correct description of what Menes had been doing in the years before adding
        # this spec, so it would be a breaking change to stop allowing them.
        | Value               | ExpectedResult    |
        | "https://myuri.com" | https://myuri.com |
        | "relativeuri"       | relativeuri       |

Scenario Outline: Parameters with valid values for simple types
    Given I have constructed the OpenAPI specification with a <ParameterLocation> parameter with name 'openApiString', type 'string', and format 'uri'
    When I try to parse the <ParameterLocation> value '<Value>' as the parameter 'openApiString'
    Then the parameter openApiString should be <ExpectedResult> of type System.Uri

    Examples:
        | ParameterLocation | Value                   | ExpectedResult    |
        | path              | https%3A%2F%2Fmyuri.com | https://myuri.com |
        | query             | https://myuri.com       | https://myuri.com |
        | cookie            | https://myuri.com       | https://myuri.com |
        | header            | https://myuri.com       | https://myuri.com |

Scenario Outline: Parameters with valid default values for simple types
    Given I have constructed the OpenAPI specification with a <ParameterLocation> parameter with name 'openApiString', type 'string', format 'uri' and default value '<DefaultValue>'
...
#
#    Examples:
#    # Note that since relative URIs are acceptable, most strings are valid. In fact I've
#    # been unable to find a string that .NET won't accept.
...
#    # URI. However, IOpenApiConverter requires incoming strings values to have the quotes
...
# But because .NET seems to accept all strings as URIs (when relative URIs are allowed)
# there are no invalid inputs. And unlike the "Incorrect body values" case, non-string
# JSON values aren't a factor because when it comes to parameters, strings are never
# quoted. Whereas our inability to distinguish between "true" and true in the body case
# is a side effect of our current code design, in this case it's actually an unavoidable
# feature: when we have, say, http://example.com/pet?name=true, the value of the name
# parameter is not necessarily the JSON boolean constant true - the example would look
# the same if the parameter is intended as a string.
