...
Feature: Date Input Parsing
    In order to implement a web API
    As a developer
    I want to be able to specify date-formatted string parameters within the OpenAPI specification and have corresponding input parameters deserialized and validated
...
Scenario Outline: Parameters with valid values for simple types
    ...
    When I try to parse the <ParameterLocation> value '<Value>' as the parameter 'openApiDate'
    Then the parameter openApiDate should be <ExpectedResult> of type System.DateTimeOffset

    Examples:
        | ParameterLocation | Value      | ExpectedResult       |
        | path              | 2017-07-21 | 2017-07-21T00:00:00Z |
...
Scenario Outline: Parameters with valid default values for simple types
    Given I have constructed the OpenAPI specification with a <ParameterLocation> parameter with name 'openApiDate', type 'string', format 'date' and default value '<DefaultValue>'
...
Scenario Outline: Incorrect parameter values
    Given I have constructed the OpenAPI specification with a <ParameterLocation> parameter with name 'openApiDate', type 'string', and format 'date'
    When I try to parse the <ParameterLocation> value '<Value>' as the parameter 'openApiDate' and expect an error
    Then an 'OpenApiBadRequestException' should be thrown
