...
Feature: Integer32 Input Parsing
    In order to implement a web API
    As a developer
    I want to be able to specify 32-bit integer parameters within the OpenAPI specification and have corresponding input parameters deserialized and validated

Scenario Outline: Parameters with valid values for simple types
    Given I have constructed the OpenAPI specification with a <ParameterLocation> parameter with name 'openApiInteger32', type 'integer', and format ''
    When I try to parse the <ParameterLocation> value '<Value>' as the parameter 'openApiInteger32'
    Then the parameter openApiInteger32 should be <ExpectedResult> of type System.Int32
...
Scenario Outline: Incorrect body values
    Given I have constructed the OpenAPI specification with a request body of type 'integer', and format ''
...
Scenario Outline: Incorrect parameter values
    Given I have constructed the OpenAPI specification with a <ParameterLocation> parameter with name 'openApiInteger', type 'integer', and format ''
...
Scenario Outline: Incorrect parameter defaults
    Given I have constructed the OpenAPI specification with a <ParameterLocation> parameter with name 'openApiInteger', type 'integer', format '' and default value '<DefaultValue>'
