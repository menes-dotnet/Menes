...
namespace Menes
{
    ...
    public readonly struct JsonDuration : IJsonValue
    {
            ...
            where T : struct, IJsonValue
        {
            ...
            if (!otherDuration.Validate(ValidationContext.ValidContext).IsValid)
            {
                return false;
            }
        ...
        public ValidationContext Validate(ValidationContext validationContext, ValidationLevel level = ValidationLevel.Flag)
        {
            if (this.HasJsonElement && (this.JsonElement.ValueKind != JsonValueKind.String || !DurationPattern.IsMatch(this.JsonElement.GetString())))
            {
                ...
                {
                    return validationContext.WithResult(isValid: false, message: $"6.1.1. type - should have been 'string' with format 'duration'.");
                }
                else
                {
