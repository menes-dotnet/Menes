...
namespace Menes
{
    ...
    public readonly struct JsonDuration : IJsonValue, IEquatable<JsonDuration>
    {
        ...
        public static implicit operator JsonAny(JsonDuration item) => JsonAny.From(item);
        ...
        public ValidationContext Validate(in ValidationContext validationContext)
        {
            ...
            if (this.HasJsonElement && !ParseDuration(this.JsonElement).Success)
            {
                return validationContext.WithError("RFC 3339 duration: the element is not convertible to a duration.");
            }
