                ...
                IEquatable<Schema>
    {
            ...
            private ValidationContext ValidateArray(JsonValueKind valueKind, in ValidationContext validationContext, ValidationLevel level)
        {
            ...
            while (arrayEnumerator.MoveNext())
            {
                        ...
                        ValidationContext containsResult = arrayEnumerator.Current.As<MinContainsDraft201909Feature.MaxContainsLessThanMinContains.Schema.ContainsEntity>().Validate(result.CreateChildContext(), level);
                        ...
                        if (containsCount < 3)
            {
                if (level >= ValidationLevel.Detailed)
                {
                    result = result.WithResult(isValid: false, $"6.4.5. minContains - {containsCount} is less than minimum number of matching items 3.");
                }
                else if (level >= ValidationLevel.Basic)
                {
                    result = result.WithResult(isValid: false, "6.4.5. minContains - item count is less than minimum number of matching items 3.");
                }
                else
                {
                    return result.WithResult(isValid: false);
