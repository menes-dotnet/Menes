...
return result;
Menes.ValidationContext ValidateNot(in RootEntity.FooEntity that, in Menes.ValidationContext validationContext, Menes.ValidationLevel level = Menes.ValidationLevel.Flag)
{
Menes.ValidationContext result = validationContext;
var notValue = that.AsJsonAny();
...
{
    notResult = notValue.Validate(validationContext.CreateChildContext("#/properties/foo/not"), level);
}
...
public bool Is<T>()
    where T : struct, Menes.IJsonValue
{
if (typeof(T) == typeof(RootEntity.FooEntity.NotEntity))
{
    return this.Validate(Menes.ValidationContext.ValidContext).IsValid;
}
    return this.As<T>().Validate(Menes.ValidationContext.ValidContext).IsValid;
