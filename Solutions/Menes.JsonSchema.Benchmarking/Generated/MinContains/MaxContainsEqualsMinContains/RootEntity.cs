...
namespace MinContainsFeature.MaxContainsEqualsMinContains
{
        ...
        public Menes.ValidationContext Validate(in Menes.ValidationContext validationContext, Menes.ValidationLevel level = Menes.ValidationLevel.Flag)
        {
            ...
            if (this.IsArray)
            {
                ...
                while (arrayEnumerator.MoveNext())
                {
                    ...
                    {
                        containsCount++;
                        if (level == Menes.ValidationLevel.Flag && containsCount > 2)
                        {
                            if (level >= Menes.ValidationLevel.Basic)
                            {
                                result = result.WithResult(isValid: false, message: "6.4.4.  maxContains - exceeded maximum of 2");
                            }
                            else
                            {
                ...
                if (containsCount > 2)
                {
                    if (level >= Menes.ValidationLevel.Basic)
                    {
                        result = result.WithResult(isValid: false, message: "6.4.4.  maxContains - greater than maximum of 2");
                    }
                    else
                    {
                        result = result.WithResult(isValid: false);
                ...
                if (containsCount < 2)
                {
                    if (level >= Menes.ValidationLevel.Basic)
                    {
                        result = result.WithResult(isValid: false, message: "6.4.5.  minContains - less than minimum of 2");
                    }
                    else
                    {
                        result = result.WithResult(isValid: false);
