...
namespace IfThenElseFeature.IfAppearsAtTheEndWhenSerializedKeywordProcessingSequence
{
...
private static readonly Menes.JsonString _MenesConstValue = new Menes.JsonString("yes");
...
if (level >= Menes.ValidationLevel.Basic)
{
    result = result.WithResult(isValid: false, message: "6.1.3. const - does not match const value \"yes\"");
}
else
{
    result = result.WithResult(isValid: false);
...
private readonly System.Text.Json.JsonElement _menesJsonElementBacking;
private readonly Menes.JsonString? _menesStringTypeBacking;
private static readonly Menes.JsonString _MenesConstValue = new Menes.JsonString("other");
...
if (level >= Menes.ValidationLevel.Basic)
{
    result = result.WithResult(isValid: false, message: "6.1.3. const - does not match const value \"other\"");
}
else
{
    result = result.WithResult(isValid: false);
