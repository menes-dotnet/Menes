...
namespace PropertyNamesFeature.PropertyNamesValidation
{
...
if (this.IsObject)
{
    ...
    {
var propertyName = new Menes.JsonString(property.NameAsMemory).As<RootEntity.PropertyNamesEntity>();
...
{
    result = result.WithResult(isValid: false, message: "6.3.1.  maxLength - value must have length less than or equal to 3");
}
else
{
...
public bool Is<T>()
    where T : struct, Menes.IJsonValue
{
if (typeof(T) == typeof(RootEntity.PropertyNamesEntity))
{
    return this.Validate(Menes.ValidationContext.ValidContext).IsValid;
}
    return this.As<T>().Validate(Menes.ValidationContext.ValidContext).IsValid;
