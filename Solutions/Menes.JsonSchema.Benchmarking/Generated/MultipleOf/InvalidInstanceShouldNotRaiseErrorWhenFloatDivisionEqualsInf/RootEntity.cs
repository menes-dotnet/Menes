...
namespace MultipleOfFeature.InvalidInstanceShouldNotRaiseErrorWhenFloatDivisionEqualsInf
{
    public readonly struct RootEntity : Menes.IJsonValue
    {
        ...
        public Menes.ValidationContext Validate(in Menes.ValidationContext validationContext, Menes.ValidationLevel level = Menes.ValidationLevel.Flag)
        {
            ...
            if (this.IsNumber)
            {
                ...
                if (System.Math.Abs(System.Math.IEEERemainder((double)number, (double)0.123456789)) > 1.0E-18)
                {
                    if (level >= Menes.ValidationLevel.Basic)
                    {
                        result = result.WithResult(isValid: false, message: "6.2.1.  multipleOf - item must be a multiple of 0.123456789");
                    }
                    else
                    {
                        result = result.WithResult(isValid: false);
