...
namespace DependentSchemasFeature.BooleanSubschemas
{
            ...
            if (this.IsObject)
            {
                foreach (var property in this.EnumerateObject())
                {
                    if (property.NameEquals("foo"))
                    {
                        ...
                        result = this.As<Menes.JsonAny>().Validate(result, level);
                    }
                    if (property.NameEquals("bar"))
                    {
                        result = result.WithLocalProperty(property.Name);
                        result = this.As<Menes.JsonNotAny>().Validate(result, level);
                    }
                }
            }
            return result;
