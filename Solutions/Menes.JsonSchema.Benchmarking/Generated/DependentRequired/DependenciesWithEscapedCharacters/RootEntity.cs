...
namespace DependentRequiredFeature.DependenciesWithEscapedCharacters
{
    public readonly struct RootEntity : Menes.IJsonObject<RootEntity>
    {
        ...
        public Menes.ValidationContext Validate(in Menes.ValidationContext validationContext, Menes.ValidationLevel level = Menes.ValidationLevel.Flag)
        {
            ...
            if (this.IsObject)
            {
                ...
                {
                    if (!this.HasProperty("foo\rbar"))
                    {
                        if (level >= Menes.ValidationLevel.Basic)
                        {
                            result = result.WithResult(isValid: false, message: "6.5.4.  dependentRequired - because you have property foo\nbar you must have property foo\rbar.");
                        }
                        else
                        {
                ...
                if (this.HasProperty("foo\"bar"))
                {
                    if (!this.HasProperty("foo'bar"))
                    {
                        if (level >= Menes.ValidationLevel.Basic)
                        {
                            result = result.WithResult(isValid: false, message: "6.5.4.  dependentRequired - because you have property foo\"bar you must have property foo'bar.");
                        }
                        else
                        {
                            result = result.WithResult(isValid: false);
