...
namespace MaximumFeature.MaximumValidationWithUnsignedInteger
{
    public readonly struct RootEntity : Menes.IJsonValue
    {
        ...
        public Menes.ValidationContext Validate(in Menes.ValidationContext validationContext, Menes.ValidationLevel level = Menes.ValidationLevel.Flag)
        {
            ...
            if (this.IsNumber)
            {
                ...
                if (number > (double)300)
                {
                    if (level >= Menes.ValidationLevel.Basic)
                    {
                        result = result.WithResult(isValid: false, message: "6.2.2.  maximum - item must be less than or equal to 300");
                    }
                    else
                    {
