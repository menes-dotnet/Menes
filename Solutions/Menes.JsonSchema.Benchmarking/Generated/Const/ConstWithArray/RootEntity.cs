...
namespace ConstFeature.ConstWithArray
{
...
private static readonly Menes.JsonAny _MenesConstValue = new Menes.JsonAny("[{ \"foo\": \"bar\" }]");
...
if (level >= Menes.ValidationLevel.Basic)
{
    result = result.WithResult(isValid: false, message: "6.1.3. const - does not match const value [{ \"foo\": \"bar\" }]");
}
else
{
    result = result.WithResult(isValid: false);
