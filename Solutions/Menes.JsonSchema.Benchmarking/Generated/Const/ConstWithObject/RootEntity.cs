...
namespace ConstFeature.ConstWithObject
{
    public readonly struct RootEntity : Menes.IJsonObject<RootEntity>
    {
        ...
        private static readonly Menes.JsonAny _MenesConstValue = new Menes.JsonAny("{\"foo\": \"bar\", \"baz\": \"bax\"}");
        ...
        public Menes.ValidationContext Validate(in Menes.ValidationContext validationContext, Menes.ValidationLevel level = Menes.ValidationLevel.Flag)
        {
            ...
            if (!this.IsObject)
            {
                if (level >= Menes.ValidationLevel.Basic)
                {
                    result = result.WithResult(isValid: false, message: "6.1.3. const - does not match const value {\"foo\": \"bar\", \"baz\": \"bax\"}");
                }
                else
                {
                    result = result.WithResult(isValid: false);
            ...
            {
                if (!this.Equals(_MenesConstValue))
                {
                    if (level >= Menes.ValidationLevel.Basic)
                    {
                        result = result.WithResult(isValid: false, message: "6.1.3. const - does not match const value {\"foo\": \"bar\", \"baz\": \"bax\"}");
