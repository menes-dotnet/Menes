...
namespace ConstFeature.FloatAndIntegersAreEqualUpTo64BitRepresentationLimits
{
    public readonly struct RootEntity : Menes.IJsonValue
    {
        ...
        private static readonly Menes.JsonNumber _MenesConstValue = new Menes.JsonNumber((double)9007199254740992);
        ...
        public Menes.ValidationContext Validate(in Menes.ValidationContext validationContext, Menes.ValidationLevel level = Menes.ValidationLevel.Flag)
        {
            ...
            if (!this.IsNumber)
            {
                if (level >= Menes.ValidationLevel.Basic)
                {
                    result = result.WithResult(isValid: false, message: "6.1.3. const - does not match const value 9007199254740992");
                }
                else
                {
                    result = result.WithResult(isValid: false);
