...
namespace ConstFeature.ConstWithAFalseDoesNotMatchA0
{
...
private static readonly Menes.JsonAny _MenesConstValue = new Menes.JsonAny("{\"a\": false}");
...
if (level >= Menes.ValidationLevel.Basic)
{
    result = result.WithResult(isValid: false, message: "6.1.3. const - does not match const value {\"a\": false}");
}
else
{
    result = result.WithResult(isValid: false);
