...
namespace ConstFeature.ConstWithArrayFalseDoesNotMatchArray0
{
...
private static readonly Menes.JsonAny _MenesConstValue = new Menes.JsonAny("[false]");
...
if (level >= Menes.ValidationLevel.Basic)
{
    result = result.WithResult(isValid: false, message: "6.1.3. const - does not match const value [false]");
}
else
{
    result = result.WithResult(isValid: false);
...
{
    var firstEnumerator = _MenesConstValue.EnumerateArray();
    var secondEnumerator = this.EnumerateArray();
    bool failed = false;
