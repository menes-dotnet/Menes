...
namespace PatternPropertiesFeature.RegexesAreNotAnchoredByDefaultAndAreCaseSensitive
{
    public readonly struct RootEntity : Menes.IJsonObject<RootEntity>
    {
        ...
        private static readonly System.Text.RegularExpressions.Regex _MenesPatternExpression0 = new System.Text.RegularExpressions.Regex("[0-9]{2,}", System.Text.RegularExpressions.RegexOptions.Compiled);
        private static readonly System.Text.RegularExpressions.Regex _MenesPatternExpression1 = new System.Text.RegularExpressions.Regex("X_", System.Text.RegularExpressions.RegexOptions.Compiled);
        ...
        public Menes.ValidationContext Validate(in Menes.ValidationContext validationContext, Menes.ValidationLevel level = Menes.ValidationLevel.Flag)
        {
            ...
            if (this.IsObject)
            {
                foreach (var property in this.EnumerateObject())
                {
                    ...
                    {
                        result = result.WithLocalProperty(property.Name);
                        result = property.Value<Menes.JsonString>().Validate(result, level);
                        if (level == Menes.ValidationLevel.Flag && !result.IsValid)
                        {
                            return result;
                        }
