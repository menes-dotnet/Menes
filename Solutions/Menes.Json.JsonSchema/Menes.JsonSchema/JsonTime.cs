...
namespace Menes.JsonSchema
{
    ...
    public readonly struct JsonTime : IJsonValue
    {
            ...
            where T : struct, IJsonValue
        {
            ...
            if (!otherTime.Validate(ValidationContext.ValidContext).IsValid)
            {
                return false;
            }
        ...
        public ValidationContext Validate(in ValidationContext validationContext, ValidationLevel level = ValidationLevel.Flag)
        {
            if (this.HasJsonElement && (this.JsonElement.ValueKind != JsonValueKind.String || this.Parse(this.JsonElement) is null))
            {
                ...
                {
                    return validationContext.WithResult(isValid: false, message: $"6.1.1.  type - should have been 'string' with format 'time'.");
                }
                else
                {
