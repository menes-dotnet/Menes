    ...
    public readonly partial struct SecurityScheme
    {
        ...
        public readonly partial struct TypeHttpBearerEntity
        {
            private ValidationContext ValidateIfThenElse(in ValidationContext validationContext, ValidationLevel level)
            {
                ...
                ValidationContext ifResult = this.As<Menes.OpenApiSchema.V31.Document.SecurityScheme.TypeHttpBearerEntity.IfEntity>().Validate(validationContext.CreateChildContext(), level);
                if (!ifResult.IsValid)
                {
                ...
                if (ifResult.IsValid)
                {
                    ...
                    ValidationContext thenResult = this.As<Menes.OpenApiSchema.V31.Document.SecurityScheme.TypeHttpBearerEntity.ThenEntity>().Validate(validationContext.CreateChildContext(), level);
