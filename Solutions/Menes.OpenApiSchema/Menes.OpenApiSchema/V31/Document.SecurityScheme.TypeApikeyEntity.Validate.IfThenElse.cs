    ...
    public readonly partial struct SecurityScheme
    {
        ...
        public readonly partial struct TypeApikeyEntity
        {
            private ValidationContext ValidateIfThenElse(in ValidationContext validationContext, ValidationLevel level)
            {
                ...
                ValidationContext ifResult = this.As<Menes.OpenApiSchema.V31.Document.SecurityScheme.TypeApikeyEntity.IfEntity>().Validate(validationContext.CreateChildContext(), level);
                if (!ifResult.IsValid)
                {
                ...
                if (ifResult.IsValid)
                {
                    ...
                    ValidationContext thenResult = this.As<Menes.OpenApiSchema.V31.Document.SecurityScheme.TypeApikeyEntity.ThenEntity>().Validate(validationContext.CreateChildContext(), level);
