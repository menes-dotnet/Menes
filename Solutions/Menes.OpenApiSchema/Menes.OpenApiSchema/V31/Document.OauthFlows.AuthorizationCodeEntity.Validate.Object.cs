            ...
            private ValidationContext ValidateObject(JsonValueKind valueKind, in ValidationContext validationContext, ValidationLevel level)
            {
                ...
                foreach (JsonObjectProperty property in this.EnumerateObject())
                {
                    if (__TryGetCorvusLocalPropertiesValidator(property, this.HasJsonElementBacking, out ObjectPropertyValidator? propertyValidator))
                    {
                        ...
                        if ((this.HasJsonElementBacking && property.NameEquals(AuthorizationUrlUtf8JsonPropertyName.Span)) || (!this.HasJsonElementBacking && property.NameEquals(AuthorizationUrlJsonPropertyName)))
                        {
                            foundAuthorizationUrl = true;
                        }
                        else if ((this.HasJsonElementBacking && property.NameEquals(TokenUrlUtf8JsonPropertyName.Span)) || (!this.HasJsonElementBacking && property.NameEquals(TokenUrlJsonPropertyName)))
                        {
                            foundTokenUrl = true;
                        }
                        else if ((this.HasJsonElementBacking && property.NameEquals(ScopesUtf8JsonPropertyName.Span)) || (!this.HasJsonElementBacking && property.NameEquals(ScopesJsonPropertyName)))
                        {
                            foundScopes = true;
                        }
                    }
