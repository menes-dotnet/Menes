            ...
            private ValidationContext ValidateObject(JsonValueKind valueKind, in ValidationContext validationContext, ValidationLevel level)
            {
                ...
                foreach (JsonObjectProperty property in this.EnumerateObject())
                {
                    if (__TryGetCorvusLocalPropertiesValidator(property, this.HasJsonElementBacking, out ObjectPropertyValidator? propertyValidator))
                    {
                        ...
                        if ((this.HasJsonElementBacking && property.NameEquals(ValueUtf8JsonPropertyName.Span)) || (!this.HasJsonElementBacking && property.NameEquals(ValueJsonPropertyName)))
                        {
                            foundValue = true;
                        }
                        else if ((this.HasJsonElementBacking && property.NameEquals(ExternalValueUtf8JsonPropertyName.Span)) || (!this.HasJsonElementBacking && property.NameEquals(ExternalValueJsonPropertyName)))
                        {
                            foundExternalValue = true;
                        }
                    }
                ...
                if (!foundExternalValue)
                {
                    if (level >= ValidationLevel.Detailed)
                    {
                        result = result.WithResult(isValid: false, $"6.5.3. required - required property \"externalValue\" not present.");
                    }
                    else if (level >= ValidationLevel.Basic)
                    {
                        result = result.WithResult(isValid: false, "6.5.3. required - required property not present.");
