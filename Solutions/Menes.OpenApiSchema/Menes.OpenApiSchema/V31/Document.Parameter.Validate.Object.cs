        ...
        private ValidationContext ValidateObject(JsonValueKind valueKind, in ValidationContext validationContext, ValidationLevel level)
        {
            ...
            foreach (JsonObjectProperty property in this.EnumerateObject())
            {
                if (__TryGetCorvusLocalPropertiesValidator(property, this.HasJsonElementBacking, out ObjectPropertyValidator? propertyValidator))
                {
                    ...
                    else if ((this.HasJsonElementBacking && property.NameEquals(InUtf8JsonPropertyName.Span)) || (!this.HasJsonElementBacking && property.NameEquals(InJsonPropertyName)))
                    {
                        foundIn = true;
                    }
                }

                if (__TryGetCorvusDependentSchemaValidator(property, this.HasJsonElementBacking, out PropertyValidator<Parameter>? dependentSchemaValidator))
                {
                    result = result.WithLocalProperty(propertyCount);
